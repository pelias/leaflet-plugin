...
- **You can now override all bounding boxes with the `overrideBbox` option.** Previously, a search result would zoom to a bounding box instead of dropping a point marker if bounding box data was available, which made sense for large areas where showing the region was more important than its centroid. However, some people expected point markers to always be dropped. While it was possible to handle selection events yourself and write your own code to always drop markers, we felt that providing the option to do this automatically would be much easier. If the `overrideBbox` option is `true` (and  `markers` is not `false`), selecting a result will always drop a marker, regardless of whether bounding box data is available.
- **Venue and address layers will always drop markers.** We indicate venue and address results with a marker icon in the default dropdown interface, but sometimes these results provided a bounding box. This meant that markers were not dropped for these small areas, where points would make sense. As a small quality-of-life tweak we now automatically always drop a point marker for these kinds of results.
